        ...
        let delegate = match regex {
            ...
            Err(_e) => {
                ...
                match regress::Regex::with_flags(&pattern, regress::Flags::from(flags)) {
                    ...
                    // TODO combine the two error messages?
                    Err(e) => Err(e).context("Could not parse regex"),
                }
            }
        }?;
        Ok(Self {
...
mod tests {
    ...
    fn invalid_regex() {
        ...
        match EsRegex::new("*", "") {
            Ok(_) => assert!(false, "should not have parsed successfully"),
            Err(e) => assert_eq!(format!("{}", e), "Could not parse regex"),
        }
    }
}
