...
export async function getInstrumentationModule(
  ...
  try {
    ...
    // avoid attempt to bundle this require
    // eslint-disable-next-line no-eval
    const instrumentationModule = await require(
      path.join(distDir, 'server', `${INSTRUMENTATION_HOOK_FILENAME}.js`)
    )
    cachedInstrumentationModule =
      instrumentationModule.default || instrumentationModule
    return cachedInstrumentationModule
  } catch (err: unknown) {
    if (
      isError(err) &&
